...
const (
	...
	// 表示 Leader 节点一次不能向目标节点发送多条消息，只能等待一条消息被响应之后，才能发送下一条消息。
	// 当刚刚复制完快照数据、上次 MsgApp 消息被拒绝（或是发送失败）或是 Leader 节点初始化时，都会导致目标节点的 Progress 切换到该状态。
